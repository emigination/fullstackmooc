...
const resolvers = {
  ...
  Query: {
    ...
    allBooks: (_root, { author, genre }) => {
      ...
      if (author) { booksToReturn = booksToReturn.filter(book => book.author === author) }
      if (genre) { booksToReturn = booksToReturn.filter(book => book.genres.includes(genre)) }
